...
@StructDefBuilder
private func groupSettings(isParam: Bool) -> any StructDefPart {
	FieldDef("name", type: .string)
		.required(!isParam)
		.doc("Group name.")
	...
	FieldDef("site", type: isParam ? .clearable(.url) : .url)
	...
	FieldDef("start_date", type: isParam ? .clearable(.unixTimestamp) : .unixTimestamp)
		.doc("The time when the event starts (events only).")

	FieldDef("finish_date", type: isParam ? .clearable(.unixTimestamp) : .unixTimestamp)
